...
## v0.8.0 — cassettes that evolve: rebase/merge, JSON Schema → assertions, cassette doctor

v0.7 made the whole trajectory assertable. v0.8 solves the maintenance problem
that comes next: your agent's prompts and code change, and re-recording every
cassette from scratch is expensive. Now you can merge a partial re-record onto an
existing cassette, generate `answer.shape` assertions directly from a JSON Schema
or OpenAI function definition, and run a health check that catches orphan events,
missing responses, empty outputs, and other cassette rot before it breaks CI.

### Added

- **`traceplay rebase <base.jsonl> <head.jsonl>`** — merge a freshly recorded
  (possibly partial) cassette onto an existing one. Segments present in both keep
  the new response (`updated`); segments only in head are `added`; segments only
  in base are `retained` (you don't lose the steps you didn't re-record).
  `--strategy theirs|ours` controls conflict resolution, `--json` for machine
  output, `--output` to write the merged cassette.
- **`traceplay shape-from-schema <schema.json>`** — convert a JSON Schema (or an
  OpenAI function/tool `parameters` block) into an `answer.shape` assertion
  snippet. Handles `type`, `properties`, `required`, `items` (→ `$.path[*]`),
  `enum`, `pattern` (→ `matches`), `const` (→ `equals`), and nested objects.
  `--step N` targets an intermediate response, `--output` writes a YAML file.
- **`traceplay doctor <cassette.jsonl>`** — cassette health diagnostics. Detects
  orphan `tool.result` / `llm.response`, missing responses/results, empty outputs,
  missing `usage` (budget assertions would read 0), non-200 responses, `seq`
  gaps/duplicates, `agent.error` events, duplicate request hashes, and pre-v0.5
  cassettes without `semanticHash`. Errors exit 1; warnings exit 0. `--json` for
  machine output.
- **SDK**: `rebaseCassettes`, `convertJsonSchema`, `renderShapeYaml`,
  `diagnoseCassette`, `doctorExitCode`, plus their result types.

### Tests & docs

- 252 tests across 37 files (up from 212 / 34): new `rebase.test.ts` (10),
  `schema-convert.test.ts` (13), `doctor.test.ts` (16), plus SDK export checks.
- New `docs/rebase.md` guide; README What's new, command reference, and project
  structure updated; ROADMAP marks v0.8 complete.

## v0.7.0 — assert the whole trajectory: structured outputs, data-flow guarantees, shareable HTML reports

v0.6 made multi-step **replay** first-class; v0.7 makes multi-step **assertions**
first-class. You can now validate structured (JSON-mode / tool-schema) answers,
prove an answer actually consumed the values its tools returned (a hallucination
guard), run one assertion template across many data rows, and export a
self-contained HTML trajectory report for PRs.

### Added
- **Structured-output assertions — `answer.shape`.** Asserts the targeted answer
  parses as JSON and matches a lightweight schema: `required` JSONPaths that must
  exist, and `fields` mapping a JSONPath to a primitive type
  (`string`/`number`/`integer`/`boolean`/`array`/`object`/`null`) or to a detailed
  expectation (`equals`, `contains`, `matches`, `enum`). Wildcards like
  `$.items[*].id` check every element. Field regexes go through the v0.6 ReDoS
  guard. Honors the v0.6 1-based `step` selector; set `json: false` to skip the
  parse requirement.
- **Data-flow guarantee — `flow.usesResult`.** Asserts the targeted answer
  actually cites value(s) a named tool returned — catching agents that ignore
  their tools and invent an answer. By default the primitive leaves of the tool
  result are the candidate values; `fromPath` selects a specific JSONPath value,
  and `minHits` sets how many distinct values must appear (case-insensitive).
- **Data-driven / parametrized cases — `each`.** A case may carry an `each` list
  of rows; the assertion template runs once per row with `{{ key }}`
  placeholders substituted into every string field, producing a labeled result
  per row (`name #1 [key=value]`). `test` and `mutate` expand rows identically
  via the shared `suite/expand.ts`.
- **Self-contained HTML report — `traceplay test --format html --output r.html`.**
  One static file with inline CSS and no external requests: a verdict/summary
  card with pass-rate bar, and per-case a **Trajectory** timeline (event chips,
  turn, HTTP status, token counts, error flags) beside the **Assertions** list.
  All cassette/assertion text is HTML-escaped. Console/JSON/Markdown are
  unchanged.
- **SDK**: `checkAnswerShape`, `checkFlowUsesResult`, `formatHtml`,
  `buildTimeline`, `expandCase`, and `renderString`/`renderValue`/`describeRow`
  are exported; `compileUserRegex`/`isLikelyCatastrophic` are now exported too.

### Changed
- **Recorder write ordering under concurrency (§6.14).** Cassette header/event
  writes now go through a serialized write queue, so a burst of concurrent
  requests lands on disk in the same order their `seq` numbers were assigned;
  `close()` drains the queue before releasing the server, so the last
  concurrent responses are never truncated.
- **Non-JSON request bodies are observable instead of silently skipped (§6.11).**
  A non-empty body that cannot be parsed is still forwarded untouched, but the
  response now carries an `x-traceplay-skipped: non-json-body` header and
  `close()` summarizes how many bodies were not recorded.

### Tests & docs
- 212 tests across 34 files pass (was 179 across 30): new suites cover
  `answer.shape`/`flow.usesResult`, templating + `each` expansion (for both
  `test` and `mutate`), the HTML report (structure, escaping, self-containment),
  and recorder concurrency/non-JSON hardening.
- New runnable example `examples/structured/`; new guide `docs/asserting-structure.md`.
