...
| `budget.tokens` | multiply every token count ×100 | `budget.maxTokens` |
...
| Mismatch safety | Ambiguity → `409`, never a silent wrong replay; ordered replay for repeated prompts | — | — | — | Often first/any match |
...
```
recorded steps:   [continue → draft-1] [continue → draft-2] [final → done]
stateful replay:  continue ⇒ draft-1   continue ⇒ draft-2   final   ⇒ done   ✅
stateless replay: continue ⇒ draft-1   continue ⇒ draft-1   final   ⇒ done   ❌
```

At step *k* the session only considers recorded steps *k…N*, so the work shrinks as the replay progresses and a repeated prompt naturally hits its next occurrence. Use it from the SDK for fine control:

```typescript
import { createReplaySession } from 'traceplay';
const session = createReplaySession(cassette.events);
session.match(requestBody1); // → step 1, advances cursor
session.match(requestBody2); // → step 2
...
npm install
npm run build
npm test          # 176 tests, incl. ordered replay, mutation testing, layered matching, redaction & golden contracts
