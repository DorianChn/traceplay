...
## What's new in v0.8 — cassettes that evolve
...
cassette from scratch is expensive. Three new commands keep cassettes useful as
the agent evolves.

- **Rebase a partial re-record onto an existing cassette** —`traceplay rebase
  old.jsonl new.jsonl` merges a freshly recorded (possibly partial) cassette onto
  the existing one. Steps you re-recorded replace the old response (`updated`);
  new steps are `added`; steps you didn't touch are `retained` (you don't lose
  them). Conflicts default to the new recording (`--strategy ours` keeps the old):
...
- **Generate `answer.shape` assertions from a schema** —`traceplay
  shape-from-schema schema.json` converts a JSON Schema (or an OpenAI
  function/tool `parameters` block) into an `answer.shape` assertion snippet you
  can paste into a suite. Handles `type`, `properties`, `required`, `items`
  (→ `$.path[*]`), `enum`, `pattern` (→ `matches`), `const` (→ `equals`), and
  nested objects:

  ```
  $ traceplay shape-from-schema ticket.schema.json --step 1
  - kind: answer.shape
    step: 1
    required:
      - "$.ticket"
...
- **Doctor a cassette before it breaks CI** —`traceplay doctor run.jsonl`
  diagnoses cassette rot: orphan `tool.result` / `llm.response`, missing
  responses or results, empty outputs, missing `usage` (budget assertions would
  read 0), non-200 responses, `seq` gaps/duplicates, `agent.error` events, and
  pre-v0.5 cassettes without `semanticHash`. Errors exit 1 so you can gate CI on
  cassette health; warnings exit 0.

See [`docs/rebase.md`](./docs/rebase.md) for the rebase workflow and
[`docs/asserting-structure.md`](./docs/asserting-structure.md) for structured
assertions. **252 tests** across 37 files.
...
- **Prove the answer used its tools (anti-hallucination)** —`flow.usesResult`
  ...
  - kind: flow.usesResult
    tool: get_ticket      # answer must cite >= 2 values get_ticket returned
    minHits: 2
  ```
...
### `traceplay rebase` (v0.8)

Merge a freshly recorded (possibly partial) cassette onto an existing one. Steps
in both keep the new response (`updated`); steps only in the new cassette are
`added`; steps only in the old one are `retained`. The merged cassette is written
to `--output` (default `merged.jsonl`).

```bash
traceplay rebase old.jsonl new.jsonl [--output merged.jsonl] [--strategy theirs|ours] [--json]
```

- `--strategy theirs` (default): on a response conflict, keep the new recording.
- `--strategy ours`: keep the old cassette's response on conflict.
- `--json`: machine-readable summary.

Use this when you changed a prompt and re-recorded only the affected steps — the
unaffected steps stay exactly as they were.

### `traceplay shape-from-schema` (v0.8)

Convert a JSON Schema (or an OpenAI function/tool `parameters` block) into an
`answer.shape` assertion snippet. Prints YAML to stdout or writes `--output`.

```bash
traceplay shape-from-schema schema.json [--output assertions.yaml] [--step N] [--json]
```

- `--step N`: target an intermediate LLM response (1-based; default = final).
- `--json`: print the raw `{required, fields}` conversion instead of YAML.

Supported keywords: `type`, `properties`, `required`, `items` (→ `$.path[*]`),
`enum`, `pattern` (→ `matches`), `const` (→ `equals`), nested objects. Unknown
keywords are skipped.

### `traceplay doctor` (v0.8)

Diagnose a cassette's health. Catches orphan `tool.result` / `llm.response`,
missing responses/results, empty outputs, missing `usage`, non-200 responses,
`seq` gaps/duplicates, `agent.error` events, duplicate request hashes, and
pre-v0.5 cassettes without `semanticHash`.

```bash
traceplay doctor cassette.jsonl [--json]
```

Errors exit 1 (gate CI on cassette health); warnings and info exit 0. `--json`
emits the full finding list.
...
│   ├── equal.ts            # key-order-insensitive deep equality
│   ├── regex-safe.ts       # ReDoS screening for user regexes
│   ├── template.ts         # {{ key }} substitution for parametrized cases
│   └── jsonpath.ts         # minimal JSONPath evaluator
├── cassette/
│   ├── store.ts            # JSONL read/write
│   ├── normalize.ts        # provider request/response normalization (+ turn/parentId)
│   ├── rebase.ts           # merge partial re-records onto existing cassettes (v0.8)
│   └── doctor.ts           # cassette health diagnostics (v0.8)
├── recorder/
│   ├── proxy.ts            # recording HTTP proxy (loopback bind, token, callId)
│   └── forward.ts          # upstream request forwarding
├── replayer/
│   ├── server.ts           # offline replay server (stateful session / 404 / 409)
│   ├── matcher.ts          # layered matcher + ordered ReplaySession
│   └── link.ts             # shared request→response linkage
├── mutate/
│   ├── mutators.ts         # fault-injection operators (answer/tool/budget)
│   └── runner.ts           # mutation testing runner + score
├── assert/
│   ├── engine.ts           # assertion dispatcher
│   ├── judge.ts            # LLM-as-judge with disk cache + timeout
│   ├── schema.ts           # JSON Schema → answer.shape converter (v0.8)
│   └── matchers/
│       ├── tool.ts         # tool.called/order/args/forbid
│       ├── answer.ts       # answer.contains/matches/judge (+ step)
│       ├── structure.ts    # answer.shape + flow.usesResult (v0.7)
│       └── budget.ts       # budget.maxTokens/maxSteps
├── suite/
│   └── expand.ts           # expand each-row parametrized cases (shared test/mutate)
├── report/
│   ├── console.ts          # console reporter
│   ├── json.ts             # JSON reporter
│   ├── diff.ts             # semantic cassette diff (changed-in-place)
│   ├── markdown.ts         # Markdown reporter
│   ├── html.ts             # self-contained HTML report (v0.7)
│   └── timeline.ts         # compress events into a renderable timeline
├── commands/
│   ├── record.ts           # `traceplay record`
│   ├── replay.ts           # `traceplay replay`
│   ├── test.ts             # `traceplay test`
│   ├── mutate.ts           # `traceplay mutate`
│   ├── rebase.ts           # `traceplay rebase` (v0.8)
│   ├── shape-from-schema.ts # `traceplay shape-from-schema` (v0.8)
│   ├── doctor.ts           # `traceplay doctor` (v0.8)
...
npm install
npm run build
npm test          # 252 tests, incl. rebase/schema/doctor, structured/data-flow assertions, ordered replay, mutation testing, layered matching, redaction & golden contracts
