...
Cassettes that evolve: rebase/merge partial re-records, JSON Schema → answer.shape, cassette health doctor.
- [x] **Cassette rebase/merge (R8)** — `rebaseCassettes` + `traceplay rebase`: merge a freshly recorded (possibly partial) cassette onto an existing one; updated/added/retained/unchanged segments, `--strategy theirs|ours`, seq renumbering
- [x] **JSON Schema → answer.shape (R8)** — `convertJsonSchema` + `renderShapeYaml` + `traceplay shape-from-schema`: JSON Schema / OpenAI function parameters → `answer.shape` snippet (type/properties/required/items/enum/pattern/const/nested)
- [x] **Cassette health doctor** — `diagnoseCassette` + `traceplay doctor`: 14 checks (orphan events, missing responses/results, empty output, missing usage, non-200, seq gaps, agent.error, duplicate requests, pre-v0.5 semanticHash); errors exit 1
- [x] **Mojibake cleanup** — rewrote the Project structure tree with correct UTF-8 box-drawing characters; fixed historical GBK→UTF-8 artifacts in README (→, —, ≥)
- 252 tests passing

## v0.9+ — Future directions
...
- [ ] **answer.shape → JSON Schema export**: reverse direction for round-tripping with external schema tools
...
1. `npm publish` (v0.8.0) — run `make check` first; `prepublishOnly` enforces build + tests
