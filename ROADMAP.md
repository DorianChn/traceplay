...
- [ ] **Single-repo restructure (R5, deferred)**: one repository with semver tags instead of per-version snapshot folders; branches for parallel majors. Deliberately not done in v0.6 to keep independently-uploadable version folders; decide A (sequential independent repos) vs B (mono-repo + tags) before v0.7
- [ ] **Richer enhancements (R8)**: response-shape assertions, cross-step assertions, cassette merge/rebase
...
1. `npm publish` (v0.6.0) — run `make check` first; `prepublishOnly` enforces build + tests
