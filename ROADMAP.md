...
- [x] **Streaming**: record & replay OpenAI-compatible SSE streams (real-time
      passthrough + content/usage capture, chunked offline replay)
- [x] **Fuzzy replay**: similarity fallback (`replay --fuzzy --fuzzy-threshold`)
- [x] **Gemini provider**: `:generateContent` / `:streamGenerateContent`
...
- [ ] **Web UI**: `traceplay ui` — local dashboard to browse cassettes,
      inspect traces, compare runs
- [ ] **Cassette diff**: compare two cassettes (before/after a prompt change)
      and highlight behavioral differences
- [ ] **TypeScript SDK**: first-class `import { record, replay, test } from 'traceplay'`
- [ ] **Tool call recording**: MCP / function-call tool events reported through
      the proxy boundary (agent posts `tool.call` to a traceplay endpoint)
