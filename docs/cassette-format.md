...
| `turn` | number | **Since v0.6 (optional).** 0-based index of this request among the trajectory's `llm.request` events; drives ordered replay. Absent in older cassettes — order is derived from event sequence instead |
| `parentId` | string | **Since v0.6 (optional).** `id` of the previous `llm.request` in the same trajectory |
...
| `callId` | string | **Since v0.6 (optional).** Correlation id echoed by the recorder's `/__traceplay/tool.call`; a later `tool.result` must reference a registered `callId` or it is rejected |
...
otherwise-identical requests match. Since v0.5 a deterministic semantic hash
(L1) folds whitespace and sampling noise, and `--fuzzy` additionally enables
structured/token similarity (see `docs/matching.md`). Since v0.6 the default
replay is **ordered**: a stateful session walks `turn` in sequence so that a
repeated prompt matches its next occurrence; pass `--stateless` to scan the
whole cassette for every request (see `docs/multi-step.md`).
