...
## 9. The ordered replay window (v0.6)

The four layers above decide *how similar* two requests are; the ordered
session decides *which candidates are even eligible*. By default the replayer
creates one `ReplaySession` per run and keeps a cursor over recorded step
order:

- the candidate set for the k-th live request is the recorded suffix from
  step k onward — already-consumed steps are excluded;
- the layers (L0→L3) then run over that suffix exactly as described above;
- a hit advances the cursor past the matched step.

This is what lets an identical prompt occur several times: its first
occurrence matches recorded step 1, its second matches step 2, and so on. It
also bounds matching cost (the suffix shrinks each step) and makes an
out-of-order request diagnosable. When the suffix is empty the result is
`exhausted`; when a request matches only a consumed step, that is reported as
out-of-order. Pass `--stateless` (or `stateful:false`) to ignore the cursor and
scan the whole cassette, the v0.5 behavior. See `docs/multi-step.md` for the
full model, SDK usage and complexity.
