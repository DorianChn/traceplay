...
export function checkToolArgs(events: TraceEvent[], assertion: Extract<Assertion, { kind: 'tool.args' }>): AssertResult {
  ...
  for (const call of calls) {
    ...
    if (assertion.equals !== undefined) {
      const hit = matches.some((m) => deepEqual(m, assertion.equals));
      if (!hit) continue;
      return {
        status: 'pass',
        assertion,
    ...
    if (assertion.matches !== undefined) {
      let re: RegExp;
      try {
        re = compileUserRegex(assertion.matches);
      } catch (err) {
        return { status: 'fail', assertion, message: `tool.args matches: ${(err as Error).message}` };
      }
