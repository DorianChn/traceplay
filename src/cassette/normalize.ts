...
/**
 * Generate a unique event id. Uses a random suffix (not a module-level counter)
 * so two recorders started concurrently in one process can never interleave
 * ids (review §6.3). The `prefix-…` shape is preserved for readability.
 */
export function nextId(prefix: string): string {
  return `${prefix}-${Date.now().toString(36)}-${randomUUID().slice(0, 8)}`;
}

/** Retained for API back-compat; ids are now random and need no global reset. */
...
/** Multi-step trajectory context (R4), supplied by the recorder. */
export interface RequestContext {
  /** Zero-based index of this request among the trajectory's llm.request events. */
  turn?: number;
  /** Id of the previous llm.request in the same trajectory. */
  parentId?: string;
}
...
export function normalizeRequest(
  ...
  if (typeof context.turn === 'number') event.turn = context.turn;
  if (typeof context.parentId === 'string') event.parentId = context.parentId;
