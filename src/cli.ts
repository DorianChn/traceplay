...
import { runShapeFromSchema } from './commands/shape-from-schema.js';
...
Usage:
  ...
  traceplay rebase <base.jsonl> <head.jsonl> [--output merged.jsonl] [--strategy theirs|ours] [--json]
...
async function main(): Promise<number> {
  ...
  switch (command) {
    ...
    case 'rebase': {
      ...
      if (!base || !head) {
        console.error('Usage: traceplay rebase <base.jsonl> <head.jsonl> [--output merged.jsonl] [--strategy theirs|ours] [--json]');
        return 2;
      }
      const strategy = typeof flags.strategy === 'string' ? flags.strategy : undefined;
      if (strategy && strategy !== 'theirs' && strategy !== 'ours') {
        console.error(`Unknown strategy "${strategy}". Expected "theirs" or "ours".`);
        return 2;
      }
      return runRebase({
        base,
        head,
        output: typeof flags.output === 'string' ? flags.output : undefined,
        strategy: (strategy as 'theirs' | 'ours') || undefined,
        json: flags.json === true,
      });
    }

    case 'shape-from-schema': {
      const schema = positional[0];
      if (!schema) {
        console.error('Usage: traceplay shape-from-schema <schema.json> [--output assertions.yaml] [--step N] [--json]');
        return 2;
      }
      ...
      if (step !== undefined && (!Number.isInteger(step) || step < 1)) {
        console.error(`invalid --step "${stepRaw}": expected a positive integer`);
        return 2;
      }
      return runShapeFromSchema({
        schema,
        output: typeof flags.output === 'string' ? flags.output : undefined,
        step,
        json: flags.json === true,
    ...
    case 'doctor': {
      const cassette = typeof flags.cassette === 'string' ? flags.cassette : positional[0];
      if (!cassette) {
        console.error('Usage: traceplay doctor <cassette.jsonl> [--json]');
        return 2;
      }
      return runDoctor({ cassette, json: flags.json === true });
    }
