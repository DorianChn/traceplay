...
import type { CaseReport, TestSuite } from '../types.js';
...
export async function runTest(args: TestArgs): Promise<number> {
  const raw = await fs.readFile(args.suite, 'utf8');
  const suite: TestSuite = args.suite.endsWith('.json')
    ? (JSON.parse(raw) as TestSuite)
    : (parseYaml(raw) as TestSuite);

  const suiteDir = dirname(isAbsolute(args.suite) ? args.suite : resolve(process.cwd(), args.suite));
  ...
  const report = buildReport(suite.suite, caseReports);
