...
export async function startRecorder(options: RecorderOptions): Promise<RecorderHandle> {
  ...
  // §6.14: serialize cassette writes so concurrent requests land on disk in the
  // same order their seq numbers were assigned (appendFile alone does not
  // guarantee ordering across interleaved async handlers).
  let writeChain: Promise<void> = Promise.resolve();
  function orderedWrite(task: () => Promise<void>): Promise<void> {
    const next = writeChain.then(task, task);
    // Keep the chain alive even if this write rejects; callers still receive
    // their own rejection via `next`.
    writeChain = next.catch(() => undefined);
    return next;
  }
  // §6.11: count request bodies we could not record so the user is told,
  // instead of silently losing traffic.
  ...
  const server = http.createServer(async (req, res) => {
    try {
      ...
      if (options.recordTools !== false && req.url && req.url.startsWith('/__traceplay/tool.')) {
        ...
        if (!headerWritten) {
          await orderedWrite(() => writeHeader(options.cassettePath, meta));
          headerWritten = true;
        }
        if (req.url === '/__traceplay/tool.call') {
          ...
          await orderedWrite(() =>
            appendEvent(options.cassettePath, {
              id: nextId('tool'),
              seq: seq++,
      ...
      // §6.11: a non-empty body we cannot parse is still forwarded, but flagged
      // on the response and counted so it is never dropped silently.
      const skippedThisRequest = !normalized && rawBody.trim().length > 0;
      if (skippedThisRequest) skippedNonJson += 1;
      ...
      if (normalized) {
        ...
        await orderedWrite(() => appendEvent(options.cassettePath, normalized!.event));
      }
      ...
      if (skippedThisRequest) {
        res.setHeader('x-traceplay-skipped', 'non-json-body');
      }
      ...
      if (streaming) {
        ...
        upstream.res.on('end', async () => {
          ...
          responseNorm.event.headers = options.redact
            ...
            : upstream.headers;
          await orderedWrite(() =>
            appendEvent(options.cassettePath, responseNorm.event),
          ).catch(() => undefined);
        });
      ...
      if (normalized) {
        ...
        responseNorm.event.headers = options.redact
          ...
          : storedHeaders;
        await orderedWrite(() => appendEvent(options.cassettePath, responseNorm.event));
      }
  ...
  return new Promise((resolve, reject) => {
    ...
    server.listen(options.port, bindHost, () => {
      resolve({
        ...
        host: bindHost,
        close: async () => {
          // Drain queued cassette writes before releasing the server so the
          // last concurrent responses are never truncated (§6.14).
          ...
          if (skippedNonJson > 0) {
            console.warn(
              `[traceplay] warning: ${skippedNonJson} request body/bodies were not JSON and were forwarded but NOT recorded (§6.11); responses carried x-traceplay-skipped.`,
            );
          }
        },
      });
