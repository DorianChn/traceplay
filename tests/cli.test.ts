...
import { parseArgs, parsePort, parseScore } from '../src/cli.js';
...
describe('cli/parseScore', () => {
  it('returns undefined when no value is given', () => {
    expect(parseScore(undefined, '--fuzzy-threshold')).toBeUndefined();
    expect(parseScore(true, '--fuzzy-threshold')).toBeUndefined();
  });

  it('parses a valid 0..1 probability', () => {
    expect(parseScore('0', '--x')).toBe(0);
    expect(parseScore('0.55', '--x')).toBeCloseTo(0.55);
    expect(parseScore('1', '--x')).toBe(1);
  });

  it('rejects non-numeric and out-of-range scores', () => {
    expect(() => parseScore('abc', '--ambiguity-gap')).toThrow(/invalid --ambiguity-gap/);
    expect(() => parseScore('-0.1', '--x')).toThrow(/invalid/);
    expect(() => parseScore('1.2', '--x')).toThrow(/invalid/);
  });
});
